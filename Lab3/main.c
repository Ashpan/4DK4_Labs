...
int main(void)
{
   ...
   */
  printf("channels\t hangup time\t\t dropped\t\t wait\t\t\t erlang\n");
  ...
  while ((random_seed = RANDOM_SEEDS[j++]) != 0) {
    ...
    while((arrival_rate = ARRIVAL_RATES[p++]) != 0) {
      ...
      while ((channels = CHANNELS[x++]) != 0) {
        unsigned HANG_UP_DURATIONS[] = {ARRAY_OF_HANG_UP_DURATIONS, 0};
        int hangup_time;
        int y=0;
        while ((hangup_time = HANG_UP_DURATIONS[y++]) != 0) {
          ...
          schedule_call_arrival_event(simulation_run,
            ...
            exponential_generator((double) 1/arrival_rate), channels, arrival_rate, hangup_time);
          ...
          output_results_excel(simulation_run, channels, arrival_rate, hangup_time);
          /* Clean up memory. */
