...
int main(void)
{
    ...
    while ((random_seed = RANDOM_SEEDS[j++]) != 0)
    {
        ...
        schedule_packet_arrival_event(simulation_run,
                                      simulation_run_get_time(simulation_run), 3);

        /*
         * Execute events until we are finished.
         */

        while ((data.number_of_packets_processed_1 + data.number_of_packets_processed_2 + data.number_of_packets_processed_3)
                < RUNLENGTH)
        {
