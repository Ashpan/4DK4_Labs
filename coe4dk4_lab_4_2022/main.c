...
main(void)
{
  ...
  float arrival_rate;
  float ARRIVAL_RATES[] = {ARRIVAL_RATE_LIST, 0};
  ...
  while ((random_seed = RANDOM_SEEDS[j++]) != 0) {
    int k = 0;
    while ((arrival_rate = ARRIVAL_RATES[k++]) != 0) {
      ...
      schedule_packet_arrival_event(simulation_run, 
          ...
          exponential_generator((double) 1/arrival_rate), arrival_rate);
